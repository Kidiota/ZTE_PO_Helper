...
while i < len(filesName):
    ...
    with pdfplumber.open(fileFullName) as pdf:
        ...
        #计算总价开始位置
        totalAmountStart = endPageText.find(' CENT ONLY ') + 11

        #计算总价结束位置
        totalAmountEnd = endPageText.find('Should the Sales Tax/Service Tax value stated in the Purchase Order differs from the Supplier')
